...
# Color Code Legend
# -------------------------------
st.subheader("Score Criteria")
legend_cols = st.columns(len(levels))
for col, (a, b, label) in zip(legend_cols, levels):
    with col:
        st.markdown(
            f"""<div style='background-color:{colors[label]}; color:white; padding:8px; border-radius:6px; text-align:center;'>
            <b>{label}</b><br>{a}–{b}%
            </div>""",
